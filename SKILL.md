...
不在 git 仓库 → `main`。一个 session 一个 thread，不需要显式参数。
...
| 其他任何字符串（含空） | 当作新 objective | Flow A |

要看状态、列 thread、清状态用 shell：

```bash
cat .claude/goal/<thread>.json    # 看进度
ls .claude/goal/                  # 列所有 thread
rm .claude/goal/<thread>.json     # 清掉
```
...
## Flow A：新 objective `/goal <objective>`
...
8. **决策**：
   ...
   - audit 不通过 → 启动续推：
     ...
     ```
   - 提示用户："Goal 已开始自动续推（thread: `<id>`）。按 ESC 中断当前 turn 即停掉续推链条。看进度：`cat .claude/goal/<id>.json`。"

## Flow B：续推 `/goal continue`（由 /loop 自动 fire）

⚠️ 几乎总是被 /loop 触发。

1. **解析 thread_id**（自动从 git branch 派生）。Read `path = .claude/goal/<thread_id>.json`。
...
2. **检查用户改话题**：扫最近 3 条用户消息（不算 `/goal continue` 自触发）。如果用户明确说了和 goal 无关的话（"先帮我看下 X"、"等等"、问无关问题），**自动 pause**：
   ...
   - **Write 一次 state.json**
   - 报告 "Goal `<id>` 已自动 pause（话题变更）。重新跑 `/goal <obj>` 启动新 goal，或 `rm .claude/goal/<id>.json` 清掉。"
...
- **thread_id 也是锁**：Flow B 自动派生 thread_id，只读写当前 thread 的文件。
...
2. ESC（中断当前 turn）→ 链条断
3. `cat .claude/goal/main.json` 看进度
4. `rm .claude/goal/main.json` 清掉
